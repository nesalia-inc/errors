...
| 0013 | Entity-First Naming                              | Bare `-er` suffixes (`Manager`, `Service`, `Handler`) are refused as standalone names; qualified suffixes (`CancelOrderHandler`) are permitted; entity names (`OrderCancellation`) are preferred. |
